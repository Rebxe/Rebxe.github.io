...
(function() {
  ...
  function fillSearchInput() {
	...
	function decryptAES(encrypted) {
		try {
			...
			// 使用正确的MD5生成密钥 - CryptoJS API使用
			const key = CryptoJS.MD5(getkey());
			...
			showError("Decryption failed. Please check the key.");
			return null;
		}
	}
	// 异步 GET 请求
		...
		xhr.onreadystatechange = function() {
		  ...
		  if (xhr.readyState === 4) {
			if (xhr.status === 200) {
			  ...
			  console.log("after:   "+decryptAES(xhr.responseText));
			  obj.success(decryptAES(xhr.responseText))
			} else {
	...
	// 异步 GET 请求
	function get_uncode(obj,total_size) {
		total_size = (typeof total_size !== 'undefined') ? total_size : -1;
		progressBar = document.getElementById('get-progress-bar');
		let progress = 0, lstprogress = 0, bse = 0.2;
		var xhr = new XMLHttpRequest()
  ...
  function keywordHighlight(searchedContent) {
	...
	function getApi(callback) {
		get({
		  url: '../privateapisize/index.html' + '?_=' + UPDATE_TIME,
		  success: function(data) {
			get({
  ...
  function searchBy(phrase, acc, callback) {
    ...
    getApi(function(response) {
	  ...
	  {
			response.posts[i].content=HtmlUtil.Decode(response.posts[i].content);
			response.posts[i].abstract=HtmlUtil.Decode(response.posts[i].abstract);
	  }
